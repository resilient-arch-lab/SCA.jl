...
function test_1_CUDA(order::Int, ns::Int, nt::Int, ldomain, lsize::Int)
    ...
    m = Moments.UniVarMomentsAccVecLabel{Float32, UInt8, CuArray, lsize}(order, ns, 256)

    CUDA.@profile Moments.centered_sum_update!(m, t, l)
end

function test_1_CPU(order::Int, ns::Int, nt::Int, ldomain, lsize::Int)
    t = Array(rand(Float32, nt, ns))
    l = Array(UInt8.(rand(UInt8, nt, lsize)))
    m = Moments.UniVarMomentsAccVecLabel{Float32, UInt8, Array, lsize}(order, ns, 256)

    @benchmark Moments.centered_sum_update!($m, $t, $l)
end
