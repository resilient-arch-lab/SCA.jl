...
function bench_Moments_centered_sum_update(TArray::Type = Array)
    ...
    Moments.fit_moments!(m2, t, l)
    bench_suite["Centered Sum Update (legacy)"] = @benchmarkable Moments.centered_sum_update_old!($m1, $t, $l)
    bench_suite["Centered Sum Update"] = @benchmarkable Moments.fit_moments!($m2, $t, $l)
...
function bench_Moments_centered_sum_update_vs_cpu(TArray::Type = Array)
    ...
    Moments.fit_moments!(m2, t, l)
    Moments.fit_moments!(m1, t, l)
    Moments.fit_moments!(m2, t, l)
    Moments.fit_moments!(m1, t, l)
    bench_suite["Centered Sum Update, TArray ($(TArray)) Device"] = @benchmarkable Moments.fit_moments!($m2, $t, $l)
    bench_suite["Centered Sum Update, Array Device"] = @benchmarkable Moments.fit_moments!($m1, $t, $l)
...
function bench_Moments_centered_sum_update_vs_combined(TArray::Type = Array)
    ...
    Moments.centered_sum_update_combined!(m2, t, l)
    Moments.fit_moments!(m1, t, l)
    bench_suite["Combined Centered Sum Update"] = @benchmarkable Moments.fit_moments!($m2, $t, $l)
    bench_suite["Centered Sum Update"] = @benchmarkable Moments.fit_moments!($m1, $t, $l)
    # bench_suite["Centered Sum Update 2"] = @benchmarkable Moments.centered_sum_update2!($m2, $t, $l)
...
function bench_Moments_VecLabel(TArray::Type = Array)
    ...
    # bench_suite["Centered Sum Update, 1 scalar label"] = @benchmarkable Moments.fit_moments!($m1, $t, $l[:, 1])
    bench_suite["Centered Sum Update, vec 4 label"] = @benchmarkable Moments.fit_moments!($m4, $t, $(l[:, 1:4]))  # [106μs on RX9070XT]
    bench_suite["Centered Sum Update, vec 8 label"] = @benchmarkable Moments.fit_moments!($m2, $t, $(l[:, 1:8]))  
    bench_suite["Centered Sum Update, vec 16 label"] = @benchmarkable Moments.fit_moments!($m3, $t, $l) 
    # bench_suite["Centered Sum Update, 8 scalar label"] = @benchmarkable Moments.fit_moments!.($mlist, $[view(t, :, :) for _ in 1:8], $[l[:, i] for i in 1:8])
